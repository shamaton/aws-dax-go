...
func (stub *ClientStub) NewDaxRequest(op *request.Operation, input, output interface{}, opt RequestOptions) *request.Request {
	h := request.Handlers{}
	h.Build.PushFrontNamed(request.NamedHandler{Name: "daxbak.BuildHandler", Fn: stub.build})
	h.Send.PushFrontNamed(request.NamedHandler{Name: "daxbak.SendHandler", Fn: stub.send})
